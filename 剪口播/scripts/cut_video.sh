...
# 获取视频信息（清理 ffprobe 输出的尾部逗号）
DURATION=$(ffprobe -v error -show_entries format=duration -of csv=p=0 "file:$INPUT" | tr -d ',')
...
if [ $? -eq 0 ]; then
  echo ""
  echo "✅ 已保存: $OUTPUT"
  NEW_DURATION=$(ffprobe -v error -show_entries format=duration -of csv=p=0 "file:$OUTPUT" | tr -d ',')
  NEW_BR=$(ffprobe -v error -show_entries stream=bit_rate -select_streams v:0 -of csv=p=0 "file:$OUTPUT" | tr -d ',')
