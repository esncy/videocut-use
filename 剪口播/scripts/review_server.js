...
let projectDir = process.cwd();  // 当前项目目录，可切换
...
function findVideoFileInDir(dir) {
  try {
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'POST' && req.url === '/api/save-selection') {
    ...
    req.on('end', () => {
      try {
        fs.writeFileSync(path.join(projectDir, 'saved_selection.json'), body);
  ...
  if (req.method === 'GET' && req.url === '/api/load-selection') {
    try {
      const selPath = path.join(projectDir, 'saved_selection.json');
      if (fs.existsSync(selPath)) {
        const data = fs.readFileSync(selPath, 'utf8');
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(data);
      } else {
  ...
  if (req.method === 'POST' && req.url === '/api/cut') {
    ...
    req.on('end', () => {
      try {
        ...
        // 保存删除列表到上层目录（项目根目录）
        const deleteJsonPath = path.join(projectDir, '..', 'delete_segments.json');
        fs.writeFileSync(deleteJsonPath, JSON.stringify(deleteList, null, 2));
        console.log(`📝 保存 ${deleteList.length} 个删除片段`);

        // 输出文件保存到项目根目录
        const baseName = path.basename(VIDEO_FILE, path.extname(VIDEO_FILE));
        const outputFile = path.join(projectDir, '..', `${baseName}_cut.mp4`);
        ...
        if (!fs.existsSync(scriptPath)) {
          ...
          execSync(`bash "${scriptPath}" "${VIDEO_FILE}" "${deleteJsonPath}" "${outputFile}"`, {
            stdio: 'inherit'
          });
        }
        ...
        const newDuration = parseFloat(execSync(`ffprobe -v error -show_entries format=duration -of csv=p=0 "file:${absOutput}"`).toString().trim());
    ...
    try {
      ...
      if (fs.existsSync(outputDir)) {
        for (const entry of fs.readdirSync(outputDir)) {
          const reviewPath = path.join(outputDir, entry, '剪口播', '3_审核', 'review.html');
          if (fs.existsSync(reviewPath)) {
            const videoLink = path.join(outputDir, entry, '剪口播', '3_审核', 'video.mp4');
            let videoName = entry;
            try {
              if (fs.lstatSync(videoLink).isSymbolicLink()) {
                videoName = path.basename(fs.readlinkSync(videoLink), path.extname(fs.readlinkSync(videoLink)));
              }
            } catch (e) {}
            projects.push({
              name: videoName,
              dir: entry,
              path: path.join(outputDir, entry, '剪口播', '3_审核'),
              active: path.resolve(projectDir) === path.resolve(path.join(outputDir, entry, '剪口播', '3_审核'))
            });
          }
        }
      }
      projects.sort((a, b) => b.dir.localeCompare(a.dir));
      res.writeHead(200, { 'Content-Type': 'application/json' });
  ...
  if (req.method === 'POST' && req.url === '/api/switch-project') {
    ...
    req.on('end', () => {
      try {
        ...
        if (fs.existsSync(path.join(absPath, 'review.html'))) {
          projectDir = absPath;
          VIDEO_FILE = findVideoFileInDir(absPath) || VIDEO_FILE;
          console.log(`🔄 切换项目: ${absPath}`);
          res.writeHead(200, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ success: true }));
        } else {
          res.writeHead(404, { 'Content-Type': 'application/json' });
  ...
  // API: 打开目录（项目根目录，即上层目录）
  if (req.method === 'POST' && req.url === '/api/open-directory') {
    const { execSync } = require('child_process');
    const dir = path.resolve(projectDir, '..');
    try {
      execSync(`open "${dir}"`, { encoding: 'utf8' });
      res.writeHead(200, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ success: true }));
    } catch (e) {
      res.writeHead(500, { 'Content-Type': 'application/json' });
...
function executeFFmpegCut(input, deleteList, output) {
  ...
  // 检测音频偏移量（MP3编码引入的延迟）
  let audioOffset = 0;
  const audioPath = path.resolve(projectDir, '..', '1_转录', 'audio.mp3');
